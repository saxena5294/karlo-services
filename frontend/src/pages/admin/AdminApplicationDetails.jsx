...
const AdminApplicationDetails = () => {
  ...
  return <div className="space-y-6"><Link to="/admin/applications" className="inline-flex items-center gap-2 text-sm font-semibold text-blue-700"><ArrowLeft size={17} /> Back to applications</Link>{feedback.message && <p className={`rounded-xl px-4 py-3 text-sm ${feedback.type === "success" ? "bg-emerald-50 text-emerald-800" : "bg-rose-50 text-rose-800"}`}>{feedback.message}</p>}
    ...
    <section className="rounded-2xl border bg-white p-6 shadow-sm"><h3 className="font-bold">Document metadata</h3><div className="mt-4 grid gap-3 sm:grid-cols-2">{application.files.map((file, index) => <div key={`${file.fieldName}-${index}`} className="flex gap-3 rounded-xl border p-4"><FileText className="shrink-0 text-blue-700" /><div className="min-w-0"><p className="truncate font-semibold">{file.originalName}</p><p className="text-xs text-slate-500">{file.format?.toUpperCase()} · {formatFileSize(file.size)}</p></div></div>)}</div></section>
