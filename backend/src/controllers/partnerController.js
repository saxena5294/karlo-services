...
export const dashboardSummary = send(async (req) => marketplaceService.getPartnerDashboardSummary(req.auth.userId));
...
export const completionDocuments = send(async (req) => ({ documents: await marketplaceService.uploadCompletionDocuments({ partnerId: req.auth.userId, id: req.params.id, files: req.files || [] }) }), 201);
