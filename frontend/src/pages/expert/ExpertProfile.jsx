...
const ExpertProfile = ({ onboarding = false }) => {
  ...
  const submit = async (event) => { event.preventDefault(); setFeedback(""); try { await updateExpertProfile({ ...form, categories: splitList(form.categories), skills: splitList(form.skills) }); if (onboarding) { navigate("/approval-pending", { replace: true, state: { profile: { role: "expert", status: "pending" } } }); return; } setFeedback(status === "pending" ? "Profile saved. Admin approval is still pending." : "Profile saved."); } catch (error) { setFeedback(error.response?.data?.message || "Unable to save expert profile."); } };
