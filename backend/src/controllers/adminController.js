...
export const replaceServiceImage = send(async (req) => { const service = await adminService.replaceAdminServiceImage(req.params.id, req.file); await writeAuditLog({ req, action: "service.image_replace", entityType: "service", entityId: service._id, summary: `Replaced image for ${service.title}` }); return { service }; });
