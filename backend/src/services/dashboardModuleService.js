...
export const replyToTicket = async (userId, role, id, message) => { await getTicket(userId, role, id); const ticket = await SupportTicket.findOneAndUpdate({ _id: id, ...ticketOwner(userId, role), status: { $ne: "closed" } }, { $push: { replies: { authorUserId: userId, authorRole: role, message: clean(message, "Reply", 3000) }, statusHistory: { status: "waiting_for_user", changedBy: userId } }, $set: { status: "waiting_for_user" } }, { returnDocument: "after", runValidators: true }).lean(); if (!ticket) throw new ApiError(409, "Closed tickets cannot receive replies"); return ticket; };
export const closeTicket = async (userId, role, id) => { const ticket = await SupportTicket.findOneAndUpdate({ _id: id, ...ticketOwner(userId, role), status: "resolved" }, { $set: { status: "closed", closedAt: new Date() }, $push: { statusHistory: { status: "closed", changedBy: userId } } }, { returnDocument: "after" }).lean(); if (!ticket) throw new ApiError(409, "Only resolved tickets can be closed"); return ticket; };
...
export const adminUpdateTicket = async (adminId, id, payload) => { if (!mongoose.isValidObjectId(id)) throw new ApiError(404, "Ticket not found"); const allowed = ["status", "priority", "assignedAdminId"]; const updates = allowedPayload(payload, allowed); if (updates.status === "closed") updates.closedAt = new Date(); const update = { $set: updates }; if (updates.status) update.$push = { statusHistory: { status: updates.status, changedBy: adminId } }; const ticket = await SupportTicket.findByIdAndUpdate(id, update, { returnDocument: "after", runValidators: true }).lean(); if (!ticket) throw new ApiError(404, "Ticket not found"); return ticket; };
export const adminReplyTicket = async (adminId, id, message) => { if (!mongoose.isValidObjectId(id)) throw new ApiError(404, "Ticket not found"); const ticket = await SupportTicket.findOneAndUpdate({ _id: id, status: { $ne: "closed" } }, { $push: { replies: { authorUserId: adminId, authorRole: "admin", message: clean(message, "Reply", 3000) }, statusHistory: { status: "in_progress", changedBy: adminId } }, $set: { status: "in_progress", assignedAdminId: adminId } }, { returnDocument: "after", runValidators: true }).lean(); if (!ticket) throw new ApiError(404, "Ticket not found or closed"); return ticket; };
