...
export const listRetailers = send(async (req) => adminService.getAdminRetailers(req.query));
export const createRetailer = send(async (req) => ({ retailer: await adminService.createRetailerProfile(req.body, req.auth.userId) }), 201);
export const updateRetailer = send(async (req) => ({ retailer: await adminService.updateRetailerProfile(req.params.id, req.body) }));
